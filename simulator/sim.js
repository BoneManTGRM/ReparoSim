...
function linePlot(canvas, xs, ysSeries, labels){
  ...
  const flat = ysSeries.flat().filter(Number.isFinite);
  const minY = Math.min(...flat, 0), maxY = Math.max(...flat, 1);
...
function runOnce(){
  ...
  scatter(els.scat, [[last(engR), last(accR)], [last(engT), last(accT)]], ['Repair','Retrain']);

  const accRfin = last(accR), accTfin = last(accT);
  const eRfin = last(engR),   eTfin = last(engT);
